...
docker build -t mvogl/jupyterhub-oc-db examples/postgres-checkpoints/db

docker build -t mvogl/jupyterhub-oc \
...
docker stop jpy-db
docker rm jpy-db
docker run -d --name jpy-db mvogl/jupyterhub-oc-db
...
docker run -it --name jpy-oc2 --link jpy-db:postgres --security-opt apparmor:unconfined --cap-add MKNOD --cap-add SYS_ADMIN --device /dev/fuse mvogl/jupyterhub-oc # --security-opt apparmor:unconfined --cap-add MKNOD --cap-add SYS_ADMIN --device /dev/fuse 
